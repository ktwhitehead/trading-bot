...
class SouthXChange:
  ...
  def scp_balance(self):
    return self.scp_balance
  ...
  async def get_book(self, session):
    ...
    async with session.get(self.base_url + self.book_url) as resp:
      ...
      # current_book_buy_amount = book["BuyOrders"][0]["Amount"]
      # current_book_sell_price = "{:.9f}".format(book["SellOrders"][0]["Price"])
      # current_book_sell_amount = book["SellOrders"][0]["Amount"]
  ...
  async def buy_the_sell_price(self, amount, session):
    ...
    async with session.post(self.base_url + self.order_url, json=json_data, headers=headers) as resp:
      ...
      print("KEATON SOUTHXCHANGE")
      print(result)

      if result == "Not enough balance":
        print("KEATON FUK NOT ENOUGH BALANCE")
        sys.exit()
